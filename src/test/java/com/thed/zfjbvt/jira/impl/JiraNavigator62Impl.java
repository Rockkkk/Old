...
import com.thed.zfjbvt.jira.impl.jira62.CreateIssuePage;
import com.thed.zfjbvt.jira.impl.jira62.JiraLandingPage;
import com.thed.zfjbvt.jira.impl.jira62.LoginPage;

public class JiraNavigator62Impl implements JiraNavigator {
...
*/
	...
	public String getZephyrTestLink() {
		try{
			String testMenu =  jiraLandingPage.getZephyrTestLink();
			Assert.assertTrue(testMenu != null, "Test Menu not finding.");
			return testMenu;
		}catch(Exception e){
			return null;
		}
	...
	public String createIssue(String issueSummary, String issueType) {
		String issue = null;
		try{
			issue  = jiraLandingPage.navigateToCreateIssuePage().createIssue(issueSummary, issueType);
			Assert.assertNotNull(issue,"Creating Issue Failed...");
		}catch(Exception e){
			e.printStackTrace();
			return issue;
		}
	...
	public boolean runJellyScript(Map<String, String> values){
		try{
			Assert.assertTrue(jiraLandingPage.navigateToSystemPage(values.get("ADMIN_AUTHENTICATE_USERNAME"), values.get("ADMIN_AUTHENTICATE_PASSWORD"))
					.navigateToJellyRunnerPage()
					.executeJellyRunner(), "Jelly Script is not executed successfully.");
