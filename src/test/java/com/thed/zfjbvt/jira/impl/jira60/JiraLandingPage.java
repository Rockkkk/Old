package com.thed.zfjbvt.jira.impl.jira60;
...
*/	
	...
	/*public boolean validateTestsDropdownMenu(){
		...
		public CreateIssuePage navigateToCreateIssuePage(){
			try{
				...
				createIssueLink.click();
				CommonUtil.implicitWait(Constants.IMPLICIT_WAIT_MEDIUM);
				logger.info("Activated Create Issue Pop-up to create an issue.");
			}catch(Exception e){
				e.printStackTrace();
			}
	...
	public boolean checkAndCloseNewFeatureDialog(){
		try{
			logger.info("Checking dialog box...");
			logger.info(CommonUtil.isElementPresent("//*[@id='whats-new-dialog']/div/h2", 5000));
			if(CommonUtil.isElementPresent("//*[@id='whats-new-dialog']/div/h2", 3000)){
				dontShowChckbox.click();
				closePopUp.click();
				CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);
			}else{
				logger.info("Not Showing Close and NewFeature popup.");
			}
	 ...
	 * @return The boolean value i.e. true or false.
	 */
	public JiraAdminAddonsPage navigateToProjectPage(String userName, String password){
		try{
			CommonUtil.flash(administrationDropDownLink);
			administrationDropDownLink.click();
			CommonUtil.visibilityOfElementLocated("//*[@id='system-admin-menu-content']");
