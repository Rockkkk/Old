...
/**
 * This is Zephyr for Jira's Administration page, where admin can manage all ZFJ related actions(like: Admin can manage test status, step status, License and General configurations etc.)
 * @author Manoj
 *
 */
		...
		try{
			...
			if(workflowChckboxStatus){
				...
				Assert.assertTrue(zephyrTestMenuPage.navigateToSearchTestsPage()
						.navigateToFirstTest().checkWorkflowToolbar(workflowChckboxStatus));
			}
			else{
				JiraAdministrationPage.getInstance().leavingAdministrationPage();
				Assert.assertTrue(zephyrTestMenuPage.navigateToSearchTestsPage()
