...
*/		}catch(Exception e){
	...
	public boolean validateTestCreated(Map <String,String> values){
		...
		try{
			...
			Assert.assertTrue(xpathForGettingProject.getText().equals(testDetails.get("PROJECT")), "Not the same Project selected");
	...
	public boolean validateTestStatus(String versionName, String cycleName, String testExecStatus, String executedUser, String executedTime){
		try{
			...
			Assert.assertTrue(validateTestExecutedUser(versionName, cycleName, executedUser), "Test Execution Status is not Validated Successfully.");
			logger.info("Test Executed User is Validated Successfully in View Issue Page.");
			
			Assert.assertTrue(validateTestExecutedTime(versionName, cycleName, executedTime), "Test Execution Status is not Validated Successfully.");
...
public boolean checkIssueLinked(){
	...
	public boolean dragAndDropTeststep(int sourceStep, int destinationStep){
		try{
			...
			String destinationStepId=CommonUtil.getAttributeValue("//*[@id='project-config-steps-table']/tbody[@class='ui-sortable']/tr["+((destinationStep+1))+"]","id");
	...
	private boolean validateTestExecutedTime(String versionName, String cycleName, String executedTime){
		try{
			...
			logger.info("ex:"+CommonUtil.getTextFromUIByXpath(xpathForExecutedTime)+"act :"+executedTime);
			//Assert.assertTrue(CommonUtil.getTextFromUIByXpath(xpathForExecutedTime).equals(executedTime),"Test execution status is not matched in view issue page.");
	...
	public Map<String, String> getTestDetails(){
		Map<String, String> values= new HashMap<String, String>();
		try{
			String testID = xpathForTestId.getText();
			Assert.assertNotNull(testID, "Test Id contains Null Value.");
			values.put("TESTID", testID);
			
			String summary = xpathForGettingSummary.getText();
			Assert.assertNotNull(summary, "Test Summary contains Null Value.");
			values.put("SUMMARY", summary);
			
			String priority = xpathForGettingPriority.getText();
			Assert.assertNotNull(priority, "Test priority contains Null Value.");
			values.put("PRIORITY", priority);
			
			String assignee = xpathForGettingAssignee.getText();
			Assert.assertNotNull(assignee, "Test assignee contains Null Value.");
			values.put("ASSIGNEE", assignee);
			
			String reporter = xpathForGettingReporter.getText();
			Assert.assertNotNull(reporter, "Test reporter contains Null Value.");
			values.put("REPORTER", reporter);
			
			String createdDate = CommonUtil.getAttributeValueUsingWebElements(xpathToGetCreatedDate, "title");
			Assert.assertNotNull(createdDate, "Test createdDate contains Null Value.");
			...
			Assert.assertNotNull(updatedDate, "Test updatedDate contains Null Value.");
			values.put("UPDATED_DATE", updatedDate);
			
			String enviroment = xpathForGettingEnvironment.getText();
			Assert.assertNotNull(enviroment, "Test enviroment contains Null Value.");
			values.put("ENVIRONMENT", enviroment);
			
			String description = xpathForGettingDescription.getText();
			Assert.assertNotNull(description, "Test createdDate contains Null Value.");
	...
	/*public boolean validateTest(Map <String,String> values){
		CommonUtil.implicitWait(Constants.EXPLICIT_WAIT_MEDIUM);
		Map<String, String>testDetails = getAllTestDetails();
		String issueType="Test";
		try{
			CommonUtil.refreshPage()
			Assert.assertTrue(validatePage(testDetails.get("TESTID")), "View Issue Page is not validated.");
			String title=CommonUtil.getTitle()
			Assert.assertTrue(title.contains(testDetails.get("TESTID")) && (title.contains(testDetails.get("SUMMARY"))),"Not the same Test...");
			
			String testID = xpathForTestId.getText();
			Assert.assertNotNull(testID, "Test Id contains Null Value.");
			logger.info(testID + " validated successfully.");
			
			String project = xpathForGettingProject.getText();
			Assert.assertTrue(.equals(testDetails.get("PROJECT")), "Not the same Project selected");
			Assert.assertTrue(xpathForGettingIssueType.getText().equals(issueType),"Not a TEST");
			Assert.assertTrue(testDetails.get("SUMMARY").equals(values.get("SUMMARY")), "Not same Summary");
			Assert.assertTrue(testDetails.get("PRIORITY").equals(values.get("PRIORITY")), "Not same Priority");
...
*/
	private boolean validatePage(String testID){
		try{
			Assert.assertTrue(testID.equals(xpathForTestId.getText()), "Test ID is not matching in View Issue Page");
			Assert.assertTrue(CommonUtil.getTitle().contains(testID), "Test ID is not matching in Title of View Issue Page");
			logger.info("View Issue Page Validated Successfully.");
		}catch(Exception e){
			return false;
		}
		return true;
