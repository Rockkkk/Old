		...
		try{
			...
			logger.info("Issue type is Test");
				...
				if(selectedPriority.equals(values.get("PRIORITY"))==false){
					...
					//CommonUtil.changeToWebElement("//*[@id='priority-suggestions']//a[text()='"+values.get("PRIORITY")+"')]").click();
					CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);
				...
				//CommonUtil.changeToWebElement("//*[@id='components-suggestions']//a[text()='"+values.get("COMPONENT")+"')]").click();
				CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);
				...
				//CommonUtil.changeToWebElement("//*[@id='versions-suggestions']//a[text()='"+values.get("AFFECTED_VERSION")+"')]").click();
				CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);
				...
				if(CommonUtil.isElementPresent("//*[@id='fixVersions-textarea']")){
					...
					CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);*/
					//*[@id='fixVersions-suggestions']//a[text()='Version 1.0']
	...
	*/
	...
	public boolean validateCreateTestPage(){
		...
		try{
			...
			String issueImageDisplayed=CommonUtil.getAttributeValue("//*[@id='issue-create']/descendant::*[text()='Issue Type']/following-sibling::img", "alt");
...
public String createTest(String summary, String priority){
		try{
			Assert.assertTrue(validatePage(), "Create Test Page is not validated successfully.");
			//summary of test
			xpathToEnterSummary.clear();
			xpathToEnterSummary.sendKeys(summary);
...
private boolean validatePage(){
	try{
		CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);
		Assert.assertTrue(xpathForCreateTestConfirmation.getText().equals("Create Issue"), "Create Test Page Header is not validated.");
		logger.info("Create Test Page Header is validated successfully.");
		String projectSelected=xpathForProjectSelected.getText();
		logger.info("Project seleced is :"+projectSelected+".");
		Assert.assertTrue(xpathForIssueTypeTest.getText().equals("Test"),"Issue type is Test is not selected.");
		logger.info("Issue type Test is Selected Successfully.");
		logger.info("Create Test Page is validated successfully.");
	}catch(Exception e){
		return false;
	}
	return true;
