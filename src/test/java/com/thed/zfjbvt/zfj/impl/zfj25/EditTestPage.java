...
private Logger logger = Logger.getLogger(this.getClass());
	...
	//Xpath for entering summary in create test page.
		...
		private WebElement xpathForUpdateButton;
			...
			try{
				if(xpathForHeaderOfEditPage.isDisplayed()){
					...
					if(CommonUtil.isElementPresent("//*[@id='fixVersions-textarea']")){	
						...
						xpathToEnterFixVersion.sendKeys(Keys.BACK_SPACE);
						xpathToEnterFixVersion.sendKeys(values.get("CHANGE_FIX_VERSION_TO"),Keys.TAB);
						CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);
					...
					xpathToEnterLabel.sendKeys(Keys.BACK_SPACE);
					xpathToEnterLabel.sendKeys(values.get("CHANGE_LABEL_TO"));
					CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);
					CommonUtil.actionClass().sendKeys(Keys.TAB).perform();
					CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);
						
					//click on update test button.
					xpathForUpdateButton.click();
					...
					values.put("FIX_VERSION", values.get("CHANGE_FIX_VERSION_TO"));
