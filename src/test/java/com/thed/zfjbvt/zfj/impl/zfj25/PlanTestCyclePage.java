...
import com.thed.zfjbvt.zfj.impl.zfj25.AddTestsToCyclePage;
import com.thed.zfjbvt.zfj.impl.zfj25.CloneTestCyclePage;
import com.thed.zfjbvt.zfj.impl.zfj25.CreateNewCyclePage;
...
private Logger logger = Logger.getLogger(this.getClass());
	...
	@FindBy(xpath="//*[@id='test-cycles-tab']/h2")
	private WebElement planTestCycleHeaderTextPNXpath;
	...
	public CloneTestCyclePage navigateToCloneTestCyclePage(String versionName, String cycleName){
		try{
			...
			CommonUtil.implicitWait(Constants.IMPLICIT_WAIT_MEDIUM);
			logger.info("Navigated to Clone test Cycle page.");
	...
	public boolean executeTestDirectly(String versionName, String cycleName, String testName, String testExecStatus){
		try{
			...
			Assert.assertTrue(validatePage(), "Plan Test cycle page is not verified successfully.");
			...
			logger.info("Test executed user is : "+executedUser);
			...
			logger.info("Test executed time is : "+executedTime);
	...
	public boolean getDataFromCycle(){
		try{
	...
	public ExecuteTestsPage navigateToExecuteTestPage(String versionName, String cycleName, String testName){
		try{
			...
			try{
				Assert.assertTrue(expandCycle(cycleName), "Version is not selected successfully.");
				CommonUtil.javaWait(Constants.JAVA_WAIT_MEDIUM);
				Assert.assertTrue(navigateToExecuteTestPage(cycleName, testName), "Not Navigated properly to execute Tests page.");
				logger.info("Navigated to Execute test page.");
			}catch(Exception e){
				try{
					JavascriptExecutor js = (JavascriptExecutor) Driver.driver;
	...
	public boolean executeTestsAndValidate(String versionName, String cycleName, String testName, String testExecStatus){
		try{
           ...
           */ 
			...
			Assert.assertTrue(testExecStatus.equals(CommonUtil.getTextFromUIByXpath(xpathForTestExecutionStatusInPlanTestCycle)), "Test status is not matched in Plan Test Cycle page.");
			logger.info("Test execution status is matched in the cycle after returning to the cycle.");
	...
	public boolean validateCycle(Map<String, String> values){
		try{
			...
			Assert.assertTrue(CommonUtil.searchTheTextInList(xpathForCycleName, values.get("CYCLE_NAME")), "Cycle name is not finding in plan test cycle.");
	...
	public boolean validateCycle(String cycleName, String cycleDesc, String cycleStartDate){
		try{
			String xpathForCycleName="//ul[@id='project-panel-cycle-list-summary']/li/div/div/a[contains(@class,'versionBanner-name')]";
			...
			Assert.assertEquals(CommonUtil.getTextFromUIByXpath(xpathForTakingDescriptionFromCycle),cycleDesc, "Cycle description is not verified properly.");
			...
			Assert.assertEquals(CommonUtil.dateConversion(cycleStartDate), CommonUtil.getTextFromUIByXpath(xpathForCycleStartDate), "Cycle Start Date is not matching.");
	...
	public boolean executeTestInViewIssuePageAndValidate(String versionName, String cycleName, String testName, String testExecStatus){
		try{
			...
			Assert.assertTrue(CommonUtil.navigateBack(), "Not Navigated back to plan test cycle.");
			CommonUtil.implicitWait(Constants.IMPLICIT_WAIT_MEDIUM);
			logger.info("Navigated back to Plan Test Cycle from View Issue Page.");
			CommonUtil.javaWait(Constants.JAVA_WAIT_MEDIUM);
			
			Assert.assertTrue(validatePage(), "Plan Test Cycle Page is not validated after returning.");
			...
			logger.info("Test executed on : "+getTestExecutedTime(cycleName, testName));
	...
	public boolean verifyAddedTest(String versionName, String cycleName, String totalTests){
		try{
			...
			if(CommonUtil.isElementPresent("//div[div[a[text()='"+cycleName+"']]]/following-sibling::div/a[text()='Next']")){
				...
				String xpathForTestsInCycle="//div[div[a[text()='"+cycleName+"']]]/following-sibling::div[@class='versionBanner-content']/table/tbody/tr/td[1]/a";
				logger.info(xpathForTestsInCycle);
				...
				logger.info("Total tests in "+cycleName+" : "+testsCount);
				logger.info("Total tests from filter : "+totalTests);
			...
			Assert.assertTrue(closeCycle(cycleName), "Cycle is closed successfully.");
	...
	private boolean hoverOnTestAndNavigateToExecutionPageUsingE(String cycleName, String testName) throws Exception{
		try{
			...
			logger.info("Cycle is expanded successfully.");
			//String xpathForTest="//a[text()='"+testName+"']";
			String xpathForTest = "//div[div[a[text()='"+cycleName+"']]]/following-sibling::div/descendant::a[text()='"+testName+"']";
	...
	private boolean navigateToExecuteTestPage(String cycleName, String testName){
		try{
			//CommonUtil.javaWait(Constants.JAVA_WAIT_MEDIUM);
			Assert.assertTrue(expandCycle(cycleName), "Cycle is not expanded successfully.");
			logger.info(cycleName+" is expanded successfully.");
			...
			CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);
			//String xpathForExecution="//td[a[text()='"+testName+"']]/following-sibling::td/a[text()='E']";
			String xpathForExecution = "//div[div[a[text()='"+cycleName+"']]]/following-sibling::div/descendant::td[a[text()='"+testName+"']]/following-sibling::td/a[text()='E']";
			Assert.assertTrue(CommonUtil.navigateThroughXpath(xpathForExecution), "Not Navigated to Execute Test Page.");
			CommonUtil.implicitWait(Constants.IMPLICIT_WAIT_LOW);
			logger.info("Navigated to Execute Tests page.");
		...
		try {
			//String v=xpathForCurrentVersion.getAttribute("value");
			 if(versionName.equals(xpathForCurrentVersion.getAttribute("value"))){
					...
					CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);					
					//CommonUtil.actionClass().sendKeys(Keys.ENTER);
					if(Config.getValue("PROJECT_NAVIGATION").equals("ON")){
						planTestCycleHeaderTextPNXpath.click();
					}else{
	...
	private boolean hoverInTestCycle(String cycleName){
		try{
			...
			/*String xpathForCycle="//ul[@id='project-panel-cycle-list-summary']/li/div/div[a[text()='"+cycleName+"']]";	
			//CommonUtil.actionClass().moveToElement(CommonUtil.returnWebElement(xpathForCycle)).release().build().perform();
			WebElement element = CommonUtil.changeToWebElement("xpathForCycle");
			CommonUtil.actionClass().moveToElement(element).build().perform();
			CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);
			logger.info(cycleName + " is selected sccessfully.");
		*/} catch (Exception e) {
			return false;
		}
		return true;
	...
	private boolean navigateToManageLinkInCycle(String cycleName){
		try{
			CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);
			String xpathForManageLinkOfCycle="//div[a[text()='"+cycleName+"']]/following-sibling::div/div[@class='cycle-manage']/a";
			//CommonUtil.actionClass().clickAndHold(onElement)doubleClick().build().perform();
			Assert.assertTrue(hoverInTestCycle(cycleName), "Hovering to cycle is not successful.");
			CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);
			//Assert.assertTrue(CommonUtil.navigateThroughXpath(xpathForManageLinkOfCycle),"Not clicked on manage link of the cycle");
			CommonUtil.navigateThroughXpath(xpathForManageLinkOfCycle);
			CommonUtil.javaWait(Constants.JAVA_WAIT_MEDIUM);
			logger.info("Clicked successfully on manage link of the Cycle.");
		} catch (Exception e) {
			e.printStackTrace();
			return false;
		}
	...
	private boolean searchTestInCycle(String cycleName, String testName){
		...
		return true;
		  ...
		  private int returnHeaderNum(String headerName){
				...
				try{
					...
					for (int i=0 ; i < xpathForHeadersOfIssueTable.size() ; i++){
						CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);
						if(xpathForHeadersOfIssueTable.get(i).getText().trim().equals(headerName)){
							rowNum=i+1;
							break;
						}
					}
		 ...
		 private boolean validatePage(){
			try{
				if(Config.getValue("PROJECT_NAVIGATION").equals("ON")){
					Assert.assertTrue(planTestCycleHeaderTextPNXpath.getText().equals("Test Cycles"), "Plan Test Cycle page is not verified successfully.");
				}else{
					Assert.assertTrue(planTestCycleHeaderTextXpath.getText().equals("Test Cycles"), "Plan Test Cycle page is not verified successfully.");
				}
		 ...
		 public List<String> getTestInfoFromCycle(String projectName, String versionName, String cycleName){
			 ...
			 try{
				 ...
				 //String projName= CommonUtil.getTextFromUIByXpath("//*[@id='content']/descendant::*/h1");
				 //String projName= CommonUtil.getTextFromUIByXpath("//*[@id='content']//h1/a");
