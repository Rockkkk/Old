...
public class TestSummaryPage {
	...
	@FindBy(xpath="//*[@id='test-summary-tab']/h2")
	private WebElement xpathToValidateTestSummaryPNPage;
	...
	public boolean validateTestSummaryPage(){
		try{
			...
			if(Config.getValue("PROJECT_NAVIGATION").equals("ON")){
				Assert.assertTrue(xpathToValidateTestSummaryPNPage.isDisplayed(),"Test summary page validation Failed...");
				Assert.assertTrue(xpathToValidateTestSummaryPNPage.getText().equals("Test Summary"), "Test Summary page is not verified successfully.");
			...
			System.out.println("Test Summary Page Validated Successfully.");
	...
	public boolean validateTestSummary(){
		try{
			CommonUtil.javaWait(Constants.JAVA_WAIT_MEDIUM);
			Assert.assertTrue(validateTestSummaryPage(),"Test summary page validation Failed...");
					...
					Assert.assertTrue(xpathTovalidateTestSummary.isDisplayed(),"Test Summary heading not displayed...");
						...
						for(int i=0;i<=(component.size()-1);i++){
							 ...
							 if(count>0){
								 CommonUtil.clickOnElement("//a[text()='"+componentName+"']");
								 SearchTestsPage.getInstance().validateTest(count);
						...
						for(int i=0;i<=(label.size()-1);i++){
							 ...
							 if(count>0){
								 ...
								 CommonUtil.clickOnElement("//a[text()='"+labelName+"']");
								 SearchTestsPage.getInstance().validateTest(count);
								 CommonUtil.javaWait(Constants.JAVA_WAIT_LOW);
	...
	public boolean validateSummaryCharts(){
		try{
			...
			Assert.assertTrue(validateTestSummaryPage(),"Test summary page validation Failed...");
					...
					Assert.assertTrue(Integer.parseInt(testExecuted) >0,"Total number of Tests executed is displayed as zero...");
					List<String> creationDateDisplayed=CommonUtil.getValues("//*[@id='testcases-creation-chart-id']//*[@class='x axis']//*[@class='tick']//*[@y='9']");
					String creationDate=creationDateDisplayed.get((creationDateDisplayed.size()-1));
					List<String> executionDateDisplayed=CommonUtil.getValues("//*[@id='execution-creation-chart-id']//*[@class='x axis']//*[@class='tick']//*[@y='9']");
					String executionDate=executionDateDisplayed.get((executionDateDisplayed.size()-1));
					List<String> testCreationRange=CommonUtil.getValues("//*[@id='testcases-creation-chart-id']//*[@class='y axis']//*[@class='tick']//*[@x='-3']");
					List<String> testExecutionrange=CommonUtil.getValues("//*[@id='execution-creation-chart-id']//*[@class='y axis']//*[@class='tick']//*[@x='-3']");
