...
public class ZFJAdminCustomizeStepStatusPage {
	...
	public boolean updateStepExecutionStatus(String oldStepStatus, String newStepStatus, String stepDescription){
		try{
			Assert.assertTrue(headerForStepExecutionStatusPageXpath.getText().equals("View Step Execution Statuses"), "This is not zfj Admin customize step status page.");
			logger.info("zfj admin customize test status page is verified successfully.");
			
			Assert.assertTrue(CommonUtil.searchTheTextInList(allStepExecutionStatuses, oldStepStatus), "zfj step execution status is not present in all step execution status list");
			logger.info("zfj step execution status ia present inside all step execution status.");
			
			
			String xpathForStepExecutionStatusEditButton="//td[contains(text(),'"+oldStepStatus+"')]/following-sibling::td/descendant::a[contains(text(),'Edit')]";
			CommonUtil.navigateThroughXpath(xpathForStepExecutionStatusEditButton);
			CommonUtil.implicitWait(Constants.IMPLICIT_WAIT_MEDIUM);
			...
			statusTextBoxForStepStatus.clear();
			statusTextBoxForStepStatus.sendKeys(newStepStatus);
			descriptionTextBoxForStepStatus.clear();
			descriptionTextBoxForStepStatus.sendKeys(stepDescription);
			submitButtonInEditStepExecutionStatus.click();
			CommonUtil.implicitWait(Constants.IMPLICIT_WAIT_MEDIUM);
			logger.info("Step status is updated successfully.");
			...
			Assert.assertTrue(CommonUtil.searchTheTextInList(allStepExecutionStatuses, newStepStatus), "zfj step execution status is not present in all step execution status list");
			logger.info("zfj step execution status ia present inside all step execution status.");
			
			String xpathForDescForNewStepExecutionStatus="//td[text()='"+newStepStatus+"']/following-sibling::td[1]";
			Assert.assertEquals(CommonUtil.getTextFromUIByXpath(xpathForDescForNewStepExecutionStatus), stepDescription, "Step execution status description is not matched.");
			logger.info("Step execution Status Description is verified Successfully.");
